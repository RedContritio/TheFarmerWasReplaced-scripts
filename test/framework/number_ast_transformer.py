...
class SyntaxSugarTransformer(ast.NodeTransformer):
    ...
    def __init__(self, function_names, module_names):
        ...
        # 需要注入 tick 的二元运算符
        self.bin_ops = {ast.Add, ast.Sub, ast.Mult, ast.Div, ast.FloorDiv, ast.Mod, ast.Pow}
    ...
    def visit_Call(self, node):
        ...
        if isinstance(node.func, ast.Attribute):
            ...
            if isinstance(value, ast.Call) and isinstance(value.func, ast.Attribute):
                _debug_print(f"[DEBUG TRANSFORMER] Chain detected: ?.{value.func.attr}().{func_name}(...)")
            else:
                _debug_print(f"[DEBUG TRANSFORMER] Converting method call: {value_type}.{func_name}(...) -> {func_name}({value_type}, ...)")
            ...
            if func_name in self.function_names:
                _debug_print(f"[DEBUG TRANSFORMER] Injecting tick for function call: {func_name}")
                tick_call = ast.Call(
        ...
        if isinstance(node.func, ast.Name) and node.func.id in self.function_names:
            _debug_print(f"[DEBUG TRANSFORMER] Injecting tick for function call: {node.func.id}")
            tick_call = ast.Call(
                func=ast.Name(id='_add_ticks', ctx=ast.Load()),
                args=[ast.Constant(value=1)],
                keywords=[]
    ...
    def visit_BinOp(self, node):
        ...
        if type(node.op) in self.bin_ops:
            _debug_print(f"[DEBUG TRANSFORMER] Injecting tick for BinOp: {type(node.op).__name__}")
    ...
    def visit_BoolOp(self, node):
        ...
        if type(node.op) in self.bool_ops:
            _debug_print(f"[DEBUG TRANSFORMER] Injecting tick for BoolOp: {type(node.op).__name__}")
            # 使用逗号表达式技巧：(_add_ticks(1), original_expr)[-1]
            tick_call = ast.Call(
                func=ast.Name(id='_add_ticks', ctx=ast.Load()),
                args=[ast.Constant(value=1)],
    ...
    def visit_Tuple(self, node):
        ...
        if isinstance(node.ctx, ast.Load) and len(node.elts) > 1 and not self.in_return:
            ...
            if any(isinstance(elt, ast.Call) and 
                   isinstance(elt.func, ast.Name) and 
                   elt.func.id == '_add_ticks' 
...
class NumberWrappingFinder(MetaPathFinder):
    ...
    def find_spec(self, fullname, path, target=None):
        ...
        if fullname in {'number_wrapper', 'number_ast_transformer', 
                        'framework.number_wrapper', 'framework.number_ast_transformer',
                        'framework.tick_system', 'framework.game_builtins'}:
        ...
        if spec.origin:
            ...
            _debug_print(f"[DEBUG FINDER] Module path: {module_path}, parent: {module_path.parent}, _PROJECT_DIR: {_PROJECT_DIR}")
            ...
            try:
                ...
                _debug_print(f"[DEBUG TRANSFORMER] Transforming module: {fullname} at {spec.origin}")
                spec.loader = NumberWrappingLoader(spec.loader)
            except ValueError:
                # module_path is not relative to _PROJECT_DIR
                _debug_print(f"[DEBUG TRANSFORMER] Skipping module: {fullname} at {spec.origin} (not in project directory)")
