...
def __move_in_maze_with_heuristic(area, path, sy, sx, ty, tx):
    ...
    while path_index < len(path):
        ...
        if maze_manhattan_distance(next_point, target_point) > maze_manhattan_distance(
            ...
            if not is_forbidden:
                # 尝试启发式直线探索
                found_new, backtrack, tried_dir = __try_shortcut(
                    area, curr_point, target_point
                )
                ...
                if found_new:
                    # 发现新路，重新 BFS 寻路
                    tree = maze_search(
                        area, get_pos_y(), get_pos_x(), ty, tx, False, False, False
                    )
